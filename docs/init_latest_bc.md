...
支持源版本 `soku_bc_tcn256_joint144_weather9_v2`。加载时比较完整结构清单（输入字段、天气压缩映射、动作、时序、当前编码器和融合），再严格加载全部权重；仅在内存中将版本名适配为 IQL 内部同构版本。原 BC 文件不改写，记录原版本与源文件 SHA256。旧天气词表模型不能直接加载。
