...
在 configs/iql_suika.yaml 中确认 data.directory 和 split_file 对应 BC 原数据集及固定划分（哈希必须匹配）；归一化继承BC，不重新拟合。TCN256需 training.burn_in=255。将 output.directory 指向新的目录，例如 outputs/iql_from_bc97k，避免覆盖已有IQL训练。路径相对IQL项目解析。
