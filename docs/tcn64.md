...
当前正式配置将 `training.burn_in` 改为 63，加载配置后明确选用 `model.temporal_mode=tcn64`，网络版本为 `soku_iql_tcn64_joint144_weather9_v2`。旧 `tcn` 仍代表 TCN32；缺省历史仍保持旧配置兼容，不能仅靠文件名判断模型窗口。
