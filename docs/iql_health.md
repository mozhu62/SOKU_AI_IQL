...
reward保留原计算，仅额外记录扣血差与KO奖励分量；td_target保持原单步公式。
类别的mean_td_target是单步TD估计，不能称为完整实测Return。
...
先固定候选batch，再按距离、HP差、空地、guard/hurt字段和动作类别轮流取样。
近中远阈值使用原始relative_x绝对值，HP优劣势只表示当前HP差。
覆盖清单明确列出缺失条件；不宣称覆盖全数据或可靠识别压制关系。
