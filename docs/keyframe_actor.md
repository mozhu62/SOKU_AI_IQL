...
当前配置enabled=true、changepoint_weight=32，与当前BC配置一致；保持帧权重1。Actor优势恒1实验及无预热保持现状，Neutral加权/筛选仍关闭。因此当前loss=sum(k*CE)/sum(k)，是关键帧BC，而非普通等权BC。Q/V依旧使用全部有效转移，TD跨度由`iql.n_step`决定；不引入SMDP、不改变采样/历史/网络。
