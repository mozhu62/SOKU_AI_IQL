...
export function Diagnostics({state}:{state:Row}){
  ...
  return <><h1 className="page-heading">Actor / Q / V 学习诊断</h1><p className="notice">TD EV 衡量 min(Q1,Q2) 对当前一步 TD 目标的解释程度，不是蒙特卡洛回报 EV。离线 NLL 最佳不等于实战最佳。</p>
    ...
    <Card title="最近优化器是否实际更新"><DataTable rows={['actor','critic','value'].map(key=>({id:key,name:key==='critic'?'双 Q':key==='value'?'V':'Actor',updated:t[`${key}_updated`],gradient:t[`${key}_gradient_norm`]}))} columns={[{key:'name',title:'网络'},{key:'updated',title:'实际更新',render:v=>v==null?'未记录':v?'是':'否 / 预热或 AMP 跳步'},{key:'gradient',title:'梯度范数'}]}/></Card>
