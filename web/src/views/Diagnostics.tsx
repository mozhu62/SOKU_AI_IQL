...
export function Diagnostics({state}:{state:Row}){
  ...
  return <><h1 className="page-heading">Actor / Q / V 学习诊断</h1><p className="notice">TD EV 衡量 min(Q1,Q2) 对当前 N-step TD 目标的解释程度，不是蒙特卡洛回报 EV。配置 N={state.config?.iql?.n_step??1}，最近训练实际平均 {number(t.n_step_actual_mean)} 步。切换 N 后旧 TD 指标不可直接比较。离线 NLL 最佳不等于实战最佳。</p>
