  ...
  return <section className="model-toolbar" aria-label="模型选择与加载">
    <div className="model-current">
      ...
      <div className="model-current-text">
        <small>{loaded ? `Joint144 · TCN${state.temporal?.context_frames??'未记录'} · ${state.device??'未记录'} · ${state.inference_dtype==='float32'?'FP32':state.inference_dtype??'精度未记录'}` : '尚无可运行模型'}</small>
