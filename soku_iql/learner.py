...
class Learner:
    ...
    def q_reference(self, batch):
        ...
        first = self.forward(n.target_q1, batch)[:, :-1].gather(-1, action).squeeze(-1)
        second = self.forward(n.target_q2, batch)[:, :-1].gather(-1, action).squeeze(-1)
    ...
    def train_batch(self, raw, step, diagnostics=False):
        ...
        value = self.forward(n.value, batch)[:, :-1, 0]
    ...
    def validate_batch(self, raw):
        ...
        logits = self.forward(self.networks.actor, b)[:, :-1][mask]
        labels = action[mask]
        q1 = self.forward(self.networks.q1, b)[:, :-1].gather(-1, action[..., None]).squeeze(-1)[mask]
        q2 = self.forward(self.networks.q2, b)[:, :-1].gather(-1, action[..., None]).squeeze(-1)[mask]
        v = self.forward(self.networks.value, b)[..., 0]
        target = td_target(b["reward"], b["terminal"], v[:, 1:], self.config["iql"]["gamma"])[mask]
        ...
        return dict(samples=int(mask.sum()), nll=float(F.cross_entropy(logits, labels)),
                    ...
                    q_mean=float(torch.minimum(q1, q2).mean()), v_mean=float(v[:, :-1][mask].mean()),
