...
当前正式配置已升级为 **TCN256**（255 帧前导历史），请优先阅读 [TCN256 结构、旧模型迁移与启动](docs/tcn256.md)。旧 TCN32/64 IQL 必须先显式迁移，新输出位于 `outputs/iql_suika_tcn256`。256 帧 Actor 请使用本项目更新后的实战端，不是未更新的原 BC 端。以下 TCN32 验收记录描述的是旧版本，不覆盖本次修改。
